        ...
        public static void DrawExtraSelectionOverlays_Postfix(Thing __instance)
        {
            if (__instance?.def != null)
            {
                ...
                else if (__instance.def.defName.Contains("Lighting_MURWallLight"))
                {
                    if (LightRadiusSettings.innerLight)
                    {
                        GenDraw.DrawRadiusRing(__instance.Position + new IntVec3(0, 0, 1).RotatedBy(__instance.Rotation), MURWallLight.UserSettings.radius * 0.91f - 2f);
                    }
                    if (LightRadiusSettings.outerLight)
                    {
                        GenDraw.DrawRadiusRing(__instance.Position + new IntVec3(0, 0, 1).RotatedBy(__instance.Rotation), MURWallLight.UserSettings.radius * 0.91f - 0.5f);
                    }
                }
            }
        }
            ...
            if (!ArchitectCategoryTab.InfoRect.Contains(UI.MousePositionOnUIInverted))
            {
                ...
                if (thingDef != null)
                {
                    ...
                    else if (thingDef.defName.Contains("Lighting_MURWallLight"))
                    {
                        ...
                        if (LightRadiusSettings.innerLight)
                        {
                            GenDraw.DrawRadiusRing(UI.MouseCell() + new IntVec3(0, 0, 1).RotatedBy(rotation), MURWallLight.UserSettings.radius * 0.91f - 2f);
                        }
                        if (LightRadiusSettings.outerLight)
                        {
                            GenDraw.DrawRadiusRing(UI.MouseCell() + new IntVec3(0, 0, 1).RotatedBy(rotation), MURWallLight.UserSettings.radius * 0.91f - 0.5f);
                        }
